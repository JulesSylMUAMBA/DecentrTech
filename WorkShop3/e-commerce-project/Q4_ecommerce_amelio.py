...
# Route GET pour récupérer tous les produits (avec filtre optionnel par catégorie)
@app.route('/products', methods=['GET'])
def get_products():
    db = load_database()
    category = request.args.get('category')  # Vérifie si un filtre "category" est fourni

    if category:
        filtered_products = [p for p in db["products"] if p["category"].lower() == category.lower()]
...
# Route DELETE pour supprimer un produit (avec vérification d'existence)
